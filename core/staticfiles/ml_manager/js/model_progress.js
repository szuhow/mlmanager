    ...
    async updateModelRow(modelId, row) {
        ...
        try {
            ...
            if (progressBar && data.progress_percentage !== undefined) {
                ...
                progressBar.textContent = `${currentEpoch}/${totalEpochs}`;
                
                // Show progress bar container if it was hidden
            ...
            if (data.model_status && data.model_status !== row.dataset.modelStatus) {
                ...
                const oldStatus = row.dataset.modelStatus;
                const newStatus = data.model_status;
                
                // Update the data-model-status attribute to reflect the new status
                row.dataset.modelStatus = newStatus;
                console.log('ModelProgressUpdater: Updated row status attribute to', newStatus);
                
                // Update the status cell display
                const statusCell = row.querySelector('.status-cell');
                if (statusCell) {
                    const statusBadge = statusCell.querySelector('.badge') || statusCell.querySelector('span');
                    if (statusBadge) {
                        // Update status display based on new status
                ...
                if (oldStatus === 'pending' && newStatus === 'training') {
                    ...
                    // Continue with live updates for newly started training
                    console.log('ModelProgressUpdater: Continuing with live updates for newly training model');
                }
                else if (oldStatus === 'loading' && newStatus === 'training') {
                    this.showStatusChangeNotification(modelId, 'Training Started', 'Model setup completed, training in progress');
                ...
                else if (oldStatus === 'pending' && newStatus === 'loading') {
                    // Pending -> Loading transition, show loading indicator
                    this.showStatusChangeNotification(modelId, 'Setup Started', 'Model is being prepared for training...');
                }
                else if (['completed', 'failed', 'cancelled'].includes(newStatus)) {
                    // Status changed to a final state, show notification and reload
                    const notificationTitle = newStatus === 'completed' ? 'Training Completed' : 
                                            newStatus === 'failed' ? 'Training Failed' : 'Training Cancelled';
                    const notificationMessage = newStatus === 'completed' ? 'Model training finished successfully' :
                                               newStatus === 'failed' ? 'Training encountered an error' : 'Training was cancelled';
                    
                    this.showStatusChangeNotification(modelId, notificationTitle, notificationMessage);
                    
                    // Reload page after delay to show final state
                    setTimeout(() => {
